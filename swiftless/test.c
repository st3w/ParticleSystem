...
struct PARTICLES Particle[ParticleCount];
...
void glCreateParticles(void) {
	...
	for (i = 1; i < ParticleCount; i++) {
		...
		Particle[i].Acceleration = ((((((8 - 5 + 2) * rand() % 11) + 5) - 1 + 1)
				* rand() % 11) + 1) * 0.02;
...
void glUpdateParticles(void) {
	...
	for (i = 1; i < ParticleCount; i++) {
		...
		Particle[i].Direction = Particle[i].Direction
				+ ((((((int) (0.5 - 0.1 + 0.1) * rand() % 11) + 1) - 1 + 1)
						* rand() % 11) + 1);
		...
		if (Particle[i].Ypos < -5) {
			...
			Particle[i].Blue = 1;
			Particle[i].Direction = 0;
			Particle[i].Acceleration = ((((((8 - 5 + 2) * rand() % 11) + 5) - 1
					+ 1) * rand() % 11) + 1) * 0.02;
			Particle[i].Deceleration = 0.0025;
		}
